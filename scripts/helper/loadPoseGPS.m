...
F_GPS = '../inekf/dataset/AGZ_subset/Log Files/OnboardGPS.csv';
F_X = '../inekf/dataset/AGZ_subset/Log Files/OnboardPose.csv';
...
% remove nan from  x_gps
x_gps = x_gps(~isnan(x_gps));
y_gps = y_gps(~isnan(y_gps));
z_gps = z_gps(~isnan(z_gps));
% remove nan and inf from XYZ_GPS

XYZ_GPS = XYZ_GPS(~isnan(XYZ_GPS(:,1)),:);
% XYZ_GPS = reshape(XYZ_GPS, [], 3);
